...
/**
 * One note, with no list around it.
 *
 * Split out of the page so the same markup serves two places: the full page at
 * `/inbox/[id]`, reached by a refresh or a shared link, and the dialog the
 * Inbox opens over itself. Rendering it twice from one definition is the only
 * way those two can be guaranteed to agree.
 */
export async function ConnectDetail({ id }: { id: string }) {
